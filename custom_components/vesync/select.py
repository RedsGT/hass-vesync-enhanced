...
async def _wfon_stage_preset(device, preset: str) -> bool:
    """Stage preset and its defaults. If cooking, restart with the new preset's settings.

    A preset change is an explicit choice — the preset's default temp/time are
    used (this DOES reset time progress, by design).
    """
    _wfon_pending.set(device, "preset", preset)
    defaults = _WFON_PRESET_DEFAULTS.get(preset)
    if defaults:
        temp_f, time_min = defaults
        _wfon_pending.set(device, "temp_f", temp_f)
...
_WFON_PRESET_DEFAULTS = {  # name: (temp_f, time_min) — Cosori CAF-P583S manual defaults
    "Air Fry":      (375, 15),
    "Broil":        (450, 12),
    "Roast":        (380, 30),
    "Bake":         (340, 30),
...
_WFON_PRESETS = list(_WFON_PRESET_DEFAULTS.keys())
...
class VeSyncSelectEntity(VeSyncBaseEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        # Notify other entities (numbers) so they re-evaluate value_fn (e.g. when a
        # preset selection also stages new temp/time pending values).
