...
def _wfon_cooldown_eta(device):
    ...
    from datetime import datetime, timedelta, timezone
    from . import _wfon_pending
    remaining = _wfon_pending.cooldown_seconds_remaining(device)
...
SENSORS: tuple[VeSyncSensorEntityDescription, ...] = (
    ...
    VeSyncSensorEntityDescription(
        ...
        ) if device.state.cook_status else None,
    ...
    VeSyncSensorEntityDescription(
        ...
        state_class=SensorStateClass.MEASUREMENT,
        value_fn=lambda device: device.state.current_temp if getattr(device.state, "cook_status", None) in ("cooking", "preheating", "heating", "ready", "keeping") else None,
        exists_fn=is_air_fryer,
    ),
    VeSyncSensorEntityDescription(
        key="cook_set_temp",
    ...
    VeSyncSensorEntityDescription(
        ...
        value_fn=lambda device: getattr(device.state, "cook_mode", None),
        exists_fn=is_air_fryer,
    ),
    VeSyncSensorEntityDescription(
        key="cook_start_time",
    ...
    VeSyncSensorEntityDescription(
        key="cooldown_eta",
        translation_key="cooldown_eta",
        device_class=SensorDeviceClass.TIMESTAMP,
        value_fn=_wfon_cooldown_eta,
        exists_fn=is_air_fryer,
    ),
)
